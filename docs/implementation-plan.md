...
[`architectures/01-no-framework-ssg.md`](./architectures/01-no-framework-ssg.md) の構成へ移行するための段取り。
...
3. **テストは同じフェーズで書く**（[01](./architectures/01-no-framework-ssg.md) §6）
...
- **Phase 7-2** — OGP 画像（記事・作品）と `og:image` / `og:type` / `twitter:card` / `twitter:image`
...
### 作る大きさの決め方（Phase 8 の後に作り直した）

当初は `build.ts` に「バリアント表」（本文用・ギャラリー用…）を置き、コンポーネントが名前で参照していた。
**表示サイズはコンポーネントが既に知っているのに、同じ数字が build 側にもある**という重複が生まれたため、
`width` / `height` だけで通るように組み替えた。

- `<Picture width={240} height={240} />` のように**求めた寸法がそのまま最適化の指示**になる。
  両方あれば切り抜き（実体は 2 倍）、幅だけなら幅に収める、省けば本文用（幅 700px・高さ 540px 上限）
- どの大きさを作るかは**描画時に記録**する（`manifest.ts` の `takeImageWants`）。
  マニフェストに無い寸法を求められたら控えておき、**ビルドは足りない分を作ってから描き直す**
- 求められた寸法は `.cache/images/requests.json` に残す。これがあると 2 回目以降は
  1 回の描画で済む（**実測: 冷えた状態 13.5s → 温まった状態 0.94s・描画 1 回**）
- **使われていない画像は変換されない。**下書きの記事や、記事ディレクトリに置いたまま
  本文から参照していない画像がこれに当たる
- dev も同じ仕組みで、足りない分は裏で作ってライブリロードで出し直す
...
1. **リンクカード（完了）** — 画像と同じくステージ境界を切った。**URL 一覧 → メタデータ JSON + サムネイル**
   ...
   - **描画が見るのはキャッシュだけ**（`offline`）。取得に失敗した URL は素のリンクとして出る（ビルドは落とさない）
   - dev は未取得の URL を**裏で取得**し、揃った時点でライブリロードで出し直す（Phase 8 の後に追加）。
     リクエストは待たせない（リンク先が遅いと 1 ページに何十秒もかかる）。
     **画像は本番と同じサムネイルを同じキャッシュへ作る** — dev はそこを配信しており、
     次の本番ビルドもその結果をそのまま使う。下書きの記事もここを通るので書きながら確認できる
   - サムネイルは高さ 120px の webp を自前で持ち、`/images/ogp_link/<ハッシュ>.webp` で配信する。
     実体を失った場合は画像の参照ごと落とし、文字だけのカードにする
2. **OGP 画像（完了）** — 記事と作品が個別の画像を持つ。ステージの形は画像・リンクカードと同じ
   - **作品**は作品画像 1 枚を `sharp` で額装する（文字は載せない）
   - **記事**は `satori`（React → SVG）でタイトルを描き、`sharp` で PNG にする。
     フォントは `src/assets/NotoSansCJKjp-Bold.woff`、アイコンは `public/omemoji.png`（base64 で埋め込む）。
     **日本語は `budoux` で文節に分け、`<span style="display: block">` 単位で折り返す**
     （satori に禁則処理が無く、そのまま流すと単語の途中で切れる）。
     絵柄は移植元と同じ（背景 `#d50000` の外枠 + 白いカード、タイトルを 4rem で中央寄せ）
   - **記事の画像は `twitter:image` にしか出さない。**`og:image` は共通の画像（720x720）のまま。
     通常のリンクカードにはサイトの顔を出し、Twitter だけ大きいカードでタイトルを見せる。
     出し分けはマニフェストの `kind`（`article` / `artwork`）で決まり、`Layout` だけが知っていればよい
   - `Layout` がマニフェストから引き、無ければ共通の画像へ倒す。**ページ側は何も渡さない**
   - キャッシュは Phase 6 と同方式（`.cache/og`）。キーは作品なら画像バイト列、記事ならタイトル + 素材。
     dev は生成しないため常に共通の画像
   - フォントとアイコンは**描くときだけ**読む（import に副作用を持たせない）
...
~~全記事・全作品に OGP がある~~ — **全記事・全作品の OGP 画像が実ファイルに解決すること**と、
記事の画像が `twitter:image` にだけ出て `og:image` は共通の画像に倒れること、
一覧・トップが共通の画像と `summary` に倒れることを検査している

---

## Phase 8: 開発サーバの仕上げ（完了）

1. **ライブリロード**（`scripts/live-reload.ts`）— **WebSocket**（`/__dev/reload`）+ dev が返す HTML にだけ差し込む受け手
   - **当初は SSE にしたが、遷移が詰まる原因になったため差し替えた。**SSE は HTTP の接続を
     1 本占め続け、ブラウザの同時接続数（HTTP/1.1 では 6）を削る。離れたページの接続が残ると
     次の遷移がその空きを待ち、一度詰まると手動でリロードしても直らない。
     WebSocket はこの制限の対象外。併せて `pagehide` で接続を手放す
   - **何が変わったかは伝えない。**dev はリクエストのたびにコンテンツを読み直すため、
     通知は「もう一度取りに来い」の合図で足りる（§7.9 の「依存グラフを持たない」と同じ理由）
   - 監視は `src` / `content` / `public`。1 回の保存で複数のイベントが来るので 50ms でまとめる
2. **CSS だけ別扱い** — `<link>` を差し替える。リロードするとスクロール位置と
   `<details>` の開閉が失われる。新しい `<link>` を読み込んでから古い方を外す（無スタイルの瞬間を作らない）
3. **`bun --hot` 対応** — ハイライタ（Phase 2 で実施済み）に加え、**監視と購読も `globalThis` へ退避**する。
   モジュールが再評価されるたびに監視を足すと、保存 1 回で何度も通知が飛ぶ
4. **エラー画面** — コンテンツの読み込みから描画までを 1 つの `try` で包む。
   frontmatter の書き損じ（zod の検証エラー）もブラウザに整形して返す。
   サイトのレイアウトは通さない（例外なのかページがそう描かれているのかを取り違えないため）。
   ライブリロードを差し込んであるので、直すと自動で開き直る

### テスト

通知の種類の割り当て（css / それ以外）/ 受け手の差し込み位置 / 購読した相手に通知が届くこと /
実ファイルを書き換えて監視が拾うこと。**本番の出力に受け手が混ざらないこと**はビルドのテストが見ている

5. **リクエストの固定費を削る**（Phase 8 の後に追加）
   - リンクカードの URL 収集は Markdown の再パースで、**毎リクエスト 79ms** かかっていた。
     本文が変わらなければ結果も変わらないので、本文そのものを鍵にして覚える。
     一覧・作品ページは **50ms → 5〜10ms** になった
   - **捨てられた遷移は描かない。**ブラウザは次のページへ進むと前のリクエストを中断する。
     気付かずに描き続けると、見捨てられたページの変換でいま待っているページが後ろに並ぶ。
     `request.signal` を変換の前後で見る（実測: 中断された 5 件の後ろで **582ms → 22ms**）
   - **リクエストのログを出す**（`200   4ms  /articles/tag/Krita`）。
     遅さの切り分けは、サーバとブラウザのどちらで待っているかを分けることから始まる
   - **dev もビルド済みの画像キャッシュを配信する。**原寸のままだと作品一覧 1 ページが
     **16 MB**（本番 358 KB）になり、連続遷移で転送とデコードが後続を圧迫していた。
     キャッシュに無いものだけリンクカードと同じく裏で変換する（実測 16 MB → 116 KB）

### 完了条件

起動 → 初回応答 **423ms** / Markdown 編集の反映 **約 130ms** / CSS はリロードなしで反映 — いずれも達成。
連続してページを切り替えても、待つのは最後に選んだページの変換だけ
