...
パフォーマンスとバッテリーに関しては、一般的なノート PC を大きく上回っている。例えばブラウザの閲覧、コーディングからイラスト制作に至るまで、一切カクつくことなくスムーズに作業することが出来る。また、それらの作業であれば 8 時間くらいはバッテリーが持つ（これは Linux のラップトップとしては破格である）。Apple Silicon 最大の特徴である GPU の性能も存分に引き出されており、Minecraft のバニラであれば概ね 250~300fps くらい出る[^minecraft]。
...
Chromium をインストールしたあと、 Google Sync API を取得する。具体的な手順は[https://658c196ef6.seesaa.net/article/474071017.html](https://658c196ef6.seesaa.net/article/474071017.html)に従えば問題ない。
